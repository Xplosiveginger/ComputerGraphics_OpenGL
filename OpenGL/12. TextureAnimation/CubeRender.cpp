...
{	
	...
	//Loading and creating the Texture1
	{
		...
		data = stbi_load("Grass.jpg", &width, &height, &clrChannels, STBI_rgb_alpha);
		if (data)
		{
		...
		{
			std::cout << "FAILED TO LOAD THE IMAGE! (IMAGE 1)" << std::endl;
		}
	...
	//Loading and creating the Texture2
	{
		...
		data = stbi_load("DBSZ.jpg", &width, &height, &clrChannels, STBI_rgb_alpha);
		if (data)
		{
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
			glGenerateMipmap(GL_TEXTURE_2D);
		...
		{
			std::cout << "FAILED TO LOAD THE IMAGE! (IMAGE 2)" << std::endl;
		}
		stbi_image_free(data);
	}
...
Shader RenderCube(const char* vertex, const char* fragment)
{
	Shader myShader = Shader(vertex, fragment);
